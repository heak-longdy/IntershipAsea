    ...
    public function onSave(Request $req)
    {
        ...
        try {
            ...
            foreach ($fromData as $index => $item) {
                ...
                $stockOnHandStockOutUseProduct = StockOnHand::where('product_id', $itemStock['product_id'])->where('shop_id', $itemStock['shop_id'])->first();

                $stockOnHandStockOutUseProduct->update([
                    "current_stock" => $stockOnHandStockOutUseProduct->current_stock - (int)$itemStock['qty'],
                    "remark" => $item->remark->value,
                    "request_by" => Auth::user()->id,
                    "request_by_type" => "admin"
                ]);
